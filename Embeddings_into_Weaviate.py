...
# Showing duplicate ASINs
duplicates_df.show()

# Counting the total number of duplicate ASINs
...
# Counting the total number of duplicate groups
total_duplicates = duplicates_df.count()
...
# Combining the text data columns
...
# Checking the current classes in Weaviate
...
def process_data_sequentially(df_with_embeddings, client, batch_size=1000):
    ...
    try:
        while processed < total_rows:
            ...
            # Adding a small delay to prevent overwhelming the system
            time.sleep(2)
...
def optimized_data_ingestion(df_with_embeddings, client, start_row=23000, batch_size=1000):
    """
    Optimized data ingestion starting after row 23000
    """
    print(f"Starting ingestion from row {start_row}")
    total_rows = df_with_embeddings.count()
    print(f"Total rows to process: {total_rows - start_row}")
    
    processed = start_row 
    start_time = time.time()
    
    # Configuring the optimal batch settings
    ...
    try:
        while processed < total_rows:
            # Getting the batch of data starting from row 23000
            batch_df = (df_with_embeddings
                       .offset(processed) 
                       .limit(batch_size)
                       .toPandas())
            ...
            for _, row in batch_df.iterrows():
                ...
                properties = {
                    col: (
                        int(row[col]) if col == 'unixReviewTime'
                ...
                objects_to_add.append((properties, vector))
            ...
            while retry_count < max_retries:
                try:
                    with client.batch as batch:
                        for properties, vector in objects_to_add:
                            batch.add_data_object(
                                data_object=properties,
                                class_name="Review",
                                vector=vector
                    ...
                    if retry_count == max_retries:
                        raise
                    print(f"Retry {retry_count} due to error: {str(e)}")
                    time.sleep(2)
            
            processed += len(batch_df)
            elapsed = time.time() - start_time
            rate = (processed - start_row) / elapsed
            
            print(f"\nProgress Update:")
            print(f"- Processed: {processed:,}/{total_rows:,} ({(processed-start_row)/(total_rows-start_row)*100:.4f}%)")
            print(f"- Time elapsed: {elapsed/3600:.2f} hours")
            print(f"- Rate: {rate:.2f} rows/second")
            print(f"- Estimated remaining: {(total_rows-processed)/rate/3600:.2f} hours")
